...
from morpheus_marketplace._streaming import Stream, AsyncStream, ServerSentEvent


@pytest.mark.asyncio
@pytest.mark.parametrize("sync", [True, False], ids=["sync", "async"])
async def test_basic(sync: bool, client: MorpheusMarketplace, async_client: AsyncMorpheusMarketplace) -> None:
...
async def test_data_missing_event(
    sync: bool, client: MorpheusMarketplace, async_client: AsyncMorpheusMarketplace
) -> None:
    def body() -> Iterator[bytes]:
        yield b'data: {"foo":true}\n'
        yield b"\n"
...
@pytest.mark.asyncio
@pytest.mark.parametrize("sync", [True, False], ids=["sync", "async"])
async def test_multiple_events(sync: bool, client: MorpheusMarketplace, async_client: AsyncMorpheusMarketplace) -> None:
    def body() -> Iterator[bytes]:
        yield b"event: ping\n"
        yield b"\n"
        yield b"event: completion\n"
